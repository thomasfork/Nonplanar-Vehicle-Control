'''
tait byran angle surface

generalization of frenet frame where the surface still has linear cross section
but can curve and twist arbitrarily in 3D

this is done by giving Tait-Bryan angles for the orientation
of the centerline as a function of path length, interpolating,
and integrating the tangent vector to obtain the centerline
'''
...
from vehicle_3d.surfaces.base_surface import BaseLinearCenterlineSurface, \
    BaseCenterlineSurfaceConfig, BaseCenterlineSurfaceSymRep
from vehicle_3d.utils.interp import InterpMethod, interp

@dataclass
class TaitBryanSurfaceConfig(BaseCenterlineSurfaceConfig):
    ''' configuration for Tait-Bryan angle surface'''
...
class TaitBryanAngleSurface(BaseLinearCenterlineSurface):
    ...
    def __init__(self, config: TaitBryanSurfaceConfig):
